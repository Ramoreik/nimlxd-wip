  ...
  if client.currentURL.hostname != url.hostname or
    ...
    when client is HttpClient:
      if url.scheme == "http+unix":
        ...
        var path = decodeUrl(url.hostname)
        net.connectUnix(client.socket, path)
      else:
      ...
      if url.scheme == "http+unix":
        client.socket = newAsyncSocket(AF_UNIX, SOCK_STREAM, IPPROTO_IP)
        var path = decodeUrl(url.hostname)
        await asyncnet.connectUnix(client.socket, path)
